...
export default function AboutSection() {
  const [contentRef, contentRevealed] = useScrollReveal({ delay: 200 });
  const [imageRef, imageRevealed] = useScrollReveal({ delay: 400 });
  const [titleRef, titleRevealed] = useScrollReveal({ delay: 100 });
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 z-10 relative">
        <div 
          ...
          className={`flex-1 text-center md:text-left transition-all duration-1000 ${
            contentRevealed ? 'animate-slide-in-left' : 'opacity-0 translate-x-[-60px]'
          }`}
        >
          <h2 
            ref={titleRef}
            className={`text-4xl md:text-5xl font-extrabold mb-8 bg-gradient-to-r from-blue-700 via-purple-600 to-blue-400 bg-clip-text text-transparent drop-shadow-lg transition-all duration-1000 ${
              titleRevealed ? 'animate-slide-in-bottom' : 'opacity-0 translate-y-[60px]'
            }`}
          >
            Hakkımızda
          </h2>
          <p className="text-xl text-gray-700 mb-6 font-medium hover-float">
          ...
          <p className="text-xl text-gray-700 font-medium hover-float">
        ...
        >
          <img
            ...
            className="rounded-3xl shadow-2xl w-full max-w-md object-cover border-4 border-blue-100 hover-float hover-pulse-glow"
          />
        </div>
      </div>
    </section>
